...
else ifeq ($(findstring 6.6,$(shell uname -r)),6.6)
	@printf "\033[0;31m BUILDING DRIVER for kernel 6.6\033[0m brcmfmac_6.6.y-nexmon/brcmfmac.ko (details: log/driver.log)\n"
	@ $(Q)make --trace -C /lib/modules/$(shell uname -r)/build M=$$PWD/brcmfmac_6.6.y-nexmon -j1 # >log/driver.log
else
...
gen/ucode.asm: $(FW_PATH)/ucode.bin
	...
	$(Q)$(NEXMON_ROOT)/patches/bcm43455c0/7_45_189/pi-6.6.y-nexmon_csi/buildtools/b43-v3/disassembler/b43-dasm $< $@ --arch 15 --format raw-le32 2>log/disass.log
	$(Q)$(NEXMON_ROOT)/patches/bcm43455c0/7_45_189/pi-6.6.y-nexmon_csi/buildtools/b43-v3/debug/b43-beautifier --asmfile $@ --defs $(NEXMON_ROOT)/patches/bcm43455c0/7_45_189/pi-6.6.y-nexmon_csi/buildtools/b43-v3/debug/include > tmp && mv tmp $@
...
ifeq ($(findstring 4.19,$(shell uname -r)),4.19)
	...
	$(Q)sudo insmod brcmfmac_6.6.y-nexmon/brcmfmac.ko
endif
else
...
ifeq ($(findstring 4.19,$(shell uname -r)),4.19)
	...
	@printf "\033[0;31m  CLEANING DRIVER\033[0m\n" $@
	$(Q)make -C /lib/modules/$(shell uname -r)/build M=$$PWD/brcmfmac_6.6.y-nexmon clean
endif
endif
	$(Q)rm -f BUILD_NUMBER
else ifneq ($(findstring bcm4366c0,$(FWUCODE)), )
